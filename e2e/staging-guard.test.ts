...
test('CI discovers this commit while the local default never uses git-staging', () => {
  ...
  assert.ok(workflow.includes('node scripts/smoke-deployment.mjs'))
  assert.ok(workflow.includes('PLAYWRIGHT_BASE_URL: ${{ steps.deployment.outputs.base_url }}'))
  assert.ok(!yamlCode(workflow).includes('https://rooted-homeschool-env-'), 'CI must not freeze on a shared alias')
...
test('the workflow parses JSON and never references a production secret', () => {
  const discovery = readFileSync(new URL('../scripts/smoke-deployment.mjs', import.meta.url), 'utf8')
  assert.ok(/response\.json\(\)/.test(discovery), 'the gate must parse JSON, not grep it')
  for (const prod of [
