...
export default function TodayPage() {
  ...
  return (
    <>
      ...
      {/* ═══════════════════════════════════════════════════════════
          DAY DETAIL — PlanV2 panel (flag ON only). When the new_plan_view
          flag is enabled, render today's lessons + appointments in the new
          shared panel. When OFF, the existing swipe-cards block renders
          below unchanged.
         ═══════════════════════════════════════════════════════════ */}
      {newPlanViewEnabled && !loading && (hasAnyLessons || todayAppointments.length > 0) && (
        <div className="bg-white border border-[#e8e5e0] rounded-2xl p-4">
          <DayDetailPanelV2
            ...
            kids={children.map(c => ({ id: c.id, name: c.name, color: c.color }))}
            isPartner={isPartner}
            onToggleLesson={toggleLesson}
            onEditLesson={openEdit}
            onDeleteLesson={deleteLesson}
            onRescheduleLesson={openReschedule}
            onSkipLesson={skipLesson}
            onMinutesUpdate={(id, mins) => setLessons(prev => prev.map(l => l.id === id ? { ...l, minutes_spent: mins } : l))}
            onToggleAppointment={async (appt) => {
              const token = await getToken();
              if (!token) return;
              setTodayAppointments(prev => prev.map(a => a.id === appt.id ? { ...a, completed: !a.completed } : a));
              try {
              ...
              } catch { /* optimistic — 15s poll will reconcile */ }
            }}
            onLessonChanged={(id, patch) => setLessons(prev => prev.map(l => l.id === id ? { ...l, ...patch } as Lesson : l))}
            variant="inline"
          />
        </div>
      )}
      ...
      {/* ═══════════════════════════════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════════ */}
      {!newPlanViewEnabled && hasAnyLessons && lessons.length > 0 && (() => {
