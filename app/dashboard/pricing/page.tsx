...
const features: FeatureRow[] = [
  ...
  { label: "Daily lesson tracking",          free: "✓",              founding: "✓",              standard: "✓"             },
  { label: "Family garden",                  free: "✓",              founding: "✓",              standard: "✓"             },
  { label: "Curated resources",              free: "✓",              founding: "✓",              standard: "✓"             },
  { label: "Memories",                       free: "Last 30 days",   founding: "✓ All time",     standard: "✓ All time"    },
  { label: "Photos",                         free: "50 photos",      founding: "✓ Unlimited",    standard: "✓ Unlimited"   },
  { label: "AI Family Updates",              free: "1 / month",      founding: "✓ Unlimited",    standard: "✓ Unlimited"   },
  { label: "Yearbook",                       free: "Preview only",   founding: "✓ PDF download", standard: "✓ PDF download"},
  { label: "Progress reports",               free: "—",              founding: "✓",              standard: "✓"             },
  { label: "Curriculum pacing",              free: "—",              founding: "✓",              standard: "✓"             },
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const [{ data: profile }, { count }] = await Promise.all([
        ...
        supabase.from("app_events").select("id", { count: "exact", head: true })
          .eq("user_id", user.id).in("type", ["memory_photo", "memory_project", "memory_book", "memory_field_trip", "memory_activity"]),
      ]);
      setIsPro((profile as { subscription_status?: string } | null)?.subscription_status === "active");
    ...
    <div className="max-w-4xl px-5 py-8 space-y-10">
      ...
      <div>
        ...
        <h1 className="text-2xl font-bold text-[#2d2926]" style={{ fontFamily: "var(--font-display)" }}>
          Start free. Upgrade when the magic happens.
        </h1>
        <p className="text-sm text-[#7a6f65] mt-1">
          Everything you need to run your homeschool — no credit card required.
        </p>
      </div>
      ...
      {loaded && !isPro && (
        <div className="flex items-center gap-2.5 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 w-fit">
          ...
          <p className="text-xs font-medium text-amber-800">
            Founding Family pricing · First 200 families — spots filling up
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className={`relative bg-[#fefcf9] border-2 rounded-2xl p-5 flex flex-col ${
          ...
          <div className="mb-4">
            ...
            <p className="text-xs text-[#7a6f65] leading-relaxed">Everything you need to get started.</p>
          </div>
          <div className="mb-5">
        ...
        <div className="relative bg-[#fefcf9] border-2 border-[#e8e2d9] rounded-2xl p-5 flex flex-col">
          ...
          <div className="mb-5">
            ...
            <p className="text-[10px] text-[#b5aca4] mt-0.5">$83.88/yr</p>
          </div>
          <div className="space-y-1.5 mb-6 flex-1">
            {[
              "Unlimited children",
      ...
      {/* Yearbook upgrade hook — only for free users with memories */}
      {loaded && !isPro && hasMemories && (
        <Link
          href="/upgrade"
          className="flex items-center gap-4 bg-gradient-to-r from-[#fef9e8] to-[#fef0d0] border border-[#f0dda8] rounded-2xl px-5 py-4 hover:from-[#fef0d0] hover:to-[#fde8b8] transition-colors"
        >
          <span className="text-2xl">📒</span>
          <div className="flex-1">
            <p className="text-sm font-semibold text-[#7a4a1a]">Your yearbook preview is ready — upgrade to download</p>
            <p className="text-xs text-[#a08040]">Turn your memories into a beautiful PDF yearbook.</p>
          </div>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-[#e8e2d9] bg-[#f8f5f0]">
                <th className="py-3 px-4 text-left text-xs font-semibold text-[#7a6f65] uppercase tracking-wide">Feature</th>
                <th className="py-3 px-4 text-center text-xs font-semibold text-[#7a6f65] uppercase tracking-wide">Free</th>
                <th className="py-3 px-4 text-center text-xs font-semibold text-[#5c7f63] uppercase tracking-wide">Founding</th>
                <th className="py-3 px-4 text-center text-xs font-semibold text-[#7a6f65] uppercase tracking-wide">Standard</th>
              </tr>
            </thead>
            <tbody>
              {features.map((f, i) => (
                <tr key={f.label} className={`border-b border-[#f0ede8] last:border-0 ${i % 2 === 0 ? "" : "bg-[#faf8f5]"}`}>
