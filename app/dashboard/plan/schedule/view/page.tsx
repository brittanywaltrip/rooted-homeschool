  ...
  const loadData = useCallback(async () => {
    ...
    const [{ data: projRowsRaw }, { data: pastDoneRaw }, { data: oneOffRaw }] = await Promise.all([
      projGoalIds.length > 0
        ? supabase
            .from("lessons")
            .select("id, title, completed, child_id, subjects(name, color), curriculum_goals(subject_label), scheduled_date, date, curriculum_goal_id, lesson_number, queue_position")
            .eq("user_id", effectiveUserId)
