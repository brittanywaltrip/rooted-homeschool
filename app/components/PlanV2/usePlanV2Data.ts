...
export function usePlanV2Data(opts: {
  ...
  const load = useCallback(async () => {
    ...
    let lessonReq = supabase
      .from("lessons")
      .select("id, title, lesson_number, completed, child_id, scheduled_date, date, curriculum_goal_id, hours, minutes_spent, notes, scheduled_source, continues_lesson_id, completed_at, subjects(name, color), curriculum_goals(subject_label)")
      .eq("user_id", effectiveUserId)
