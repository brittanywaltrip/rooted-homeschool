...
**Sharing.** Every resource card has a Share button next to the bookmark. It sends the line "(title), free from Rooted Homeschool App" with a link to that resource's own public page, `rootedhomeschoolapp.com/r/(slug)`; on a device with no share sheet it copies the line and link and says "Link copied". The public page needs no account: it shows the picture, the description, a "Get the free printable" button to the original site, and one short paragraph about Rooted with a Start free button. When the resource is a Rooted tool rather than an outside printable (the Fall Photo Frame), the button reads "Make yours in Rooted" and goes to signup, and after onboarding the family is sent to that tool. The slug is optional and set in the admin page ("Slug (optional)": lowercase letters, numbers and hyphens, 3 to 40 characters, not already used by another active resource); a resource without one is shared by its id and works the same. What is counted: `resource_shared` (with share or copy), `resource_landing_viewed` and `resource_landing_clicked` on the public page, and `from` and `r` on `user_signed_up` for a family who signed up from one. None of this touches partner referral codes.
...
- **Weekly summary.** Mondays at 15:00 UTC, to a family with a memory or a completed lesson dated in the last **30 days** (it was 14, which meant a family who took a fortnight off stopped hearing from Rooted at all). Two versions. A family who checked something off in the week just gone (Monday to Sunday, in their own timezone) gets counts: "Last week Zoe finished 9 lessons and Emma finished 8 lessons. You captured 2 photos and 1 win.", then where each child's tree stands and how many leaves to the next stage, then a button to Today. Subject: "Your week with Rooted: 9 lessons, 3 memories". A family who checked nothing off gets the quiet version instead ("A quiet week is still a week"), which says their plan is where they left it and shows the same garden line. One per family per week, and a family who got the win-back email in the last 7 days is skipped so the two never land together.
...
The Monday email counts what is there: lessons by the day they were checked off for, memories by their date, and leaves for the school year, the same number the Garden page shows.
