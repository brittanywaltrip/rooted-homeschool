-- Rolls 20260919235000 back to the state 20260919230000 left behind.
--
-- The earlier version of this file only dropped the NEW signature. But the
-- forward migration BEGINS by dropping the preceding one, so that rollback
-- left the database with no schedule_commit at all -- a worse state than
-- either migration, and not the "restore the previous state" it claimed.
--
-- This drops the new signature AND recreates the previous definition verbatim,
-- copied from 20260919230000. Proved by comparing pg_get_functiondef before
-- the forward migration and after this rollback: see rollback-roundtrip.sh.
--
-- WHAT THIS GIVES UP, so that it is a decision and not a surprise: the row and
-- parent locks that close the concurrent-edit and vacation-insert races, the
-- hours content guard, the cascade check, payload validation, and the FK
-- ownership checks. Callers that pass p_lesson_updates will fail with
-- "function does not exist" until they are rolled back too.

drop function if exists public.schedule_commit(uuid, jsonb, jsonb, uuid[], jsonb, jsonb, text);
